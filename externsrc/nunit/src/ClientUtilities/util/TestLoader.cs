...
namespace NUnit.Util
{
	...
	public class TestLoader : LongLivingMarshalByRefObject, NUnit.Core.EventListener, ITestLoader
	{
		...
		{
			if ( this.IsTestRunning && args.ExceptionObject.GetType() != typeof( System.Threading.ThreadAbortException ) )
...
//				return false;
		...
		public void LoadTest( string testName )
		{
			...
			{
				lastException = new ApplicationException( "A SecurityException prevented loading one or more assemblies.\r\rIf you are attempting to load an assembly from a network share, you may need to change your security settings or copy it to a local drive.", exception );
				events.FireTestLoadFailed( TestFileName, lastException );
			}
			...
			if ( IsTestRunning )
				...
				try
				{
                    ...
                    Test newTest = TestProject.IsAssemblyWrapper
                        ? newDomain.Load(testProject.ActiveConfig.Assemblies[0].FullPath, loadedTestName)
