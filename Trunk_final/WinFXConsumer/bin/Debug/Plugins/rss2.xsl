<?xml version="1.0"?>
  ...
  <xsl:template match="/">
	...
	<xsl:for-each select="rss/channel">
	  <div>
	    <p><h2 style="color:blue; margin:50"><xsl:value-of select="title"/></h2>
            </p>

	    <h4 style="color:red"><xsl:text> link:  </xsl:text><xsl:value-of select="link"/></h4>
	    <h4 style="color:red"><xsl:text> about:  </xsl:text><xsl:value-of select="description"/></h4>
	    <h4 style="color:red"><xsl:text> language: </xsl:text><xsl:value-of select="language"/></h4>
	    <h4 style="color:red"><xsl:text> generated by:  </xsl:text><xsl:value-of select="generator"/></h4>
	  </div>
	</xsl:for-each>
